    ...
    <div className="mt-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col items-center gap-6">
        {products.map((product) => {
          if (!product || !product.id || !product.title || !product.image || isNaN(product.price)) {
            return null;
          }
          ...
          return (
            ...
            >
              ...
              <div className="text-white text-center sm:text-right w-full sm:w-1/3">
                ...
                <button
                  className='mt-3 text-sm text-white border border-white rounded-lg px-2 py-1 hover:bg-white hover:text-gray-900 transition'
                  onClick={() => dispatch(RemoveOneCart({ id: product.id }))}
                >
                  Clear Product
                </button>
              </div>
      ...
      <div className="mt-10 text-center">
        <Link to="/checkout" className="bg-yellow-500 text-white py-3 px-6 rounded-full shadow-xl hover:bg-yellow-600 transition inline-block text-center">
          Proceed to Checkout
        </Link>
      </div>
    </div>
